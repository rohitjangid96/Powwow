 ...
 * @author Lenovo
 */
public class SelectInterest extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            ArrayList<Integer> keywordal=new ArrayList<>();
            for( String str: keywordarray)
            {
              str=str.substring(2);
              keywordal.add(Integer.parseInt(str));
            }
            
            KeywordDao kd=new KeywordDao();
            if(kd.insertuserkeyword(keywordal, context, session))
            {
