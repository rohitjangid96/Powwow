...
public class QuestionDao {
    ...
    public boolean insertQuestion(QuestionModel qm,ServletContext context,HttpSession session)
    {
        try {
            ...
            //keyword name using kid
            ArrayList<String> alltags=new ArrayList<>();
            for(String i: qm.getTags())
            {
                String query10="select kname from keyword where kid=?";
            
            PreparedStatement pst10;
            pst10=con.prepareStatement(query10);
            pst10.setInt(1, Integer.parseInt(i));
            ResultSet rst10=pst10.executeQuery();
            if(rst10.next())
            {
              alltags.add(rst10.getString(1));
            }
            ...
            if(ps.executeUpdate()>0)
            {
             ...
             String qr8="select kid from userkeyword where uid=? and kid in (";
    ...
    public void myQuestions(RelatedQuestionModel rqm,String id,ServletContext context)
    {
        try {
            ...
            String qr="select qid,que,did,timestamp,reportabusecount from question where uid=? order by qid desc";
            ps=con.prepareStatement(qr);
            ps.setString(1,id);
            rs=ps.executeQuery();
